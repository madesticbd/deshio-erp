...
export default function ExchangeProductModal({ sale, onClose, onExchange }: ExchangeProductModalProps) {
    ...
    const delayDebounce = setTimeout(() => {
      ...
      flattenedProducts.forEach((prod: any) => {
        ...
        availableItems.forEach((item: any) => {
          ...
          if (!groups[bid]) {
            groups[bid] = { batchId: bid, price: item.sellingPrice, count: 0 };
          }
          groups[bid].count++;
        });
  ...
  const handleAddReplacement = () => {
    ...
    if (existingIndex !== -1) {
      ...
      if (newQty > availableQty) {
        alert(`Only ${availableQty} units available. You already have ${updated[existingIndex].quantity} in cart.`);
        return;
      }
    ...
    setReplacementProducts(prev => 
      prev.map(p => {
        if (p.id === productId && p.batchId === batchId) {
          return { ...p, quantity: newQty, amount: p.price * newQty };
        }
        return p;
      })
    );
  ...
  const calculateTotals = () => {
    ...
    return { originalAmount, newSubtotal, vatRate, vatAmount, totalNewAmount, difference };
  };
                  ...
                  {selectedReplacement && (
                    <div className="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-500 dark:border-blue-600 rounded-lg">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button type="button" onClick={() => { setSelectedReplacement(null); setReplacementQuantity('1'); }} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                          <X size={18} />
                        </button>
                      </div>
                      ...
                      <div className="flex gap-3 mb-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-green-600 dark:text-green-400">Available: {getAvailableInventory(selectedReplacement.id, selectedReplacement.batchId)} units</p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={() => { const qty = parseInt(replacementQuantity) || 1; if (qty > 1) setReplacementQuantity(String(qty - 1)); }} className="w-10 h-10 flex items-center justify-center border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-lg font-bold">−</button>
                            <input type="number" min="1" value={replacementQuantity} onChange={(e) => setReplacementQuantity(e.target.value)} className="flex-1 px-4 py-2 text-center border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none text-lg font-semibold" />
                            <button type="button" onClick={() => { const qty = parseInt(replacementQuantity) || 1; setReplacementQuantity(String(qty + 1)); }} className="w-10 h-10 flex items-center justify-center border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-lg font-bold">+</button>
                          </div>
                        </div>

                        <button type="button" onClick={handleAddReplacement} className="w-full px-4 py-2.5 bg-black hover:bg-gray-800 text-white font-medium rounded-lg transition-colors">Add to Replacements</button>
                      </div>
                    </div>
                  )}
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-2">
                              <div className="flex items-center gap-1">
                                <button type="button" onClick={() => handleUpdateReplacementQty(product.id, product.batchId, product.quantity - 1)} className="w-7 h-7 flex items-center justify-center border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">−</button>
                              ...
                              <button type="button" onClick={() => handleRemoveReplacement(product.id, product.batchId)} className="text-red-600 hover:text-red-700">
                                <X size={18} />
                              </button>
                            </div>
                          </div>
                  ...
                  <div className="p-4 space-y-3">
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setShowNoteCounter(!showNoteCounter)} className="flex items-center gap-1 px-2 py-1 text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded hover:bg-blue-100 dark:hover:bg-blue-900/30">
                      ...
                      {showNoteCounter ? (
                        ...
                        <div>
                          ...
                          <input type="number" value={cashFromNotes > 0 ? cashFromNotes : cashAmount} onChange={(e) => { setCashAmount(Number(e.target.value)); setNote1000(0); setNote500(0); setNote200(0); setNote100(0); setNote50(0); setNote20(0); setNote10(0); setNote5(0); setNote2(0); setNote1(0); }} className="w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                      ...
                      <div className="flex justify-between text-base">
                        ...
                        <span className={`font-bold ${remainingBalance > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-green-600 dark:text-green-400'}`}>৳{remainingBalance.toFixed(2)}</span>
                      </div>
                      {remainingBalance > 0 && <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">Can pay later</p>}
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                      ...
                      {remainingBalance > 0 && <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">Can refund later</p>}
                    </div>
                  </div>
                </div>
              )}
