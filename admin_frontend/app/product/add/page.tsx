    ...
    try {
      ...
      if (isEditMode) {
        ...
        if (variations.length > 0) {
          ...
          const baseGroupId = `group-${Date.now()}`;
          ...
          for (const variation of variations) {
            for (const size of variation.sizes) {
              ...
              // FILTER OUT variation-specific fields to prevent duplicates
              const baseCustomFields = customFields.filter(
                cf => !VARIATION_FIELD_IDS.includes(cf.field_id)
              );

              // Create product with variation fields
              const varCustomFields = [
                ...baseCustomFields,  // User-selected fields (without variation fields)
                { field_id: 6, value: variation.color }, // Color field
                { field_id: 7, value: size }, // Size field
                { field_id: 15, value: baseGroupId }, // Variation Group
                { field_id: 16, value: '1' }, // Is Variation
              ];

              const productData = {
                name: variationName,
                sku: baseData.sku, // SAME SKU for all!
                      ...
                      <div className="border-t border-gray-300 dark:border-gray-700 pt-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {availableFields
                            ...
                            .filter(f => !['Primary Image', 'Additional Images', 'SKU', 'Product Name', 'Description', 'Category', 'Vendor', 'Color', 'Size', 'Variation Group', 'Is Variation'].includes(f.title))
                            .map((field) => (
                              <button
                                key={field.id}
                                type="button"
